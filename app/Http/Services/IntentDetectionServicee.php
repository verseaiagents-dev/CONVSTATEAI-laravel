...
class IntentDetectionServicee {
    ...
    private function getSampleCategoryRecommendations() {
        try {
            ...
            foreach ($chunks as $chunk) {
                $metadata = is_string($chunk->metadata) ? json_decode($chunk->metadata, true) ?? [] : [];
                ...
                $categories[$category]['total_price'] += $metadata['price'] ?? 0;
                $categories[$category]['total_rating'] += $metadata['rating'] ?? 4.0;
            }
    ...
    private function searchProductsByAttributes($attributes) {
        try {
            ...
            $products = [];
            foreach ($chunks as $chunk) {
                $metadata = is_string($chunk->metadata) ? json_decode($chunk->metadata, true) ?? [] : (is_array($chunk->metadata) ? $chunk->metadata : []);
                
                $products[] = [
                    'id' => $chunk->id,
                    'name' => $metadata['product_title'] ?? 'Ürün ' . $chunk->id,
                    'category' => $metadata['product_category'] ?? 'Genel',
                    'price' => $metadata['product_price'] ?? 0,
                    'brand' => 'Bilinmeyen',
                    'rating' => $metadata['product_rating']['rate'] ?? 4.0,
                    'stock' => 10,
                    'image' => ProductImageHelper::getImageWithFallback(null),
                    'description' => substr($chunk->content, 0, 200) . '...',
                    'product_url' => 'https://example.com/product/' . $chunk->id . '?intent=specific_search',
                    'source' => 'knowledge_base',
